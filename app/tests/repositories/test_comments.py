...
class CommentsRepositoryTest(unittest.TestCase):
    ...
    def test_get_existing_user_comments(self):
        result = CommentsRepository.get_user_comments(self.user_a.id)
        """The return should be an list instance"""
        self.assertIsInstance(result, list)

        """Assert that the items in the returned list are instances of Comment class"""
        self.assertTrue(
            all(isinstance(comment, Comment) for comment in result)
        )

        """Assert all the comments from the query was created by the user """
        self.assertEqual(
            all((comment.get('author').get('id') == self.user_a.id) for comment in result), True
        )
